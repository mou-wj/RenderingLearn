...
void ViewBRDFAndSample(){
	...
	//绘制采样点
	for(uint sampleIndex = 0;sampleIndex < 20;sampleIndex++)
	{
		...
		if(wiDotWm != 0)
		{
			...
			finalColor = vec4(0,ft_p_wo_wi * wiDotWm * 0.5,0,1);
		}
	...
	for(uint sampleIndex = 0;sampleIndex < 20;sampleIndex++)
	{
		...
		thetas =  deltaThetaFine.x + theta_t;
		fines = deltaThetaFine.y + fine_t;
