...
//计算在经过p次内部折射而出的光线的gamma值为gamma_o的入射光的phi值
float Phi(float p,float phi_o, float phi_t){
	return  2 * p * phi_t - 2 * phi_o + p * s_pi;
}

//Np  Hair Azimuthal Scattering Function   入射光折射光的插值在在法平面圆周上的分布,分布类似与均值为0的正态分布 
float Np(float phi/*-pi 到 pi*/,float roughness){
	while(phi > s_pi){
		phi -=2 * s_pi;
	}
	while(phi < -s_pi){
	...
	//简单用一个正太分布来控制分布，可变的参数为方差,用粗擦度表示,这里实际的取值只会在-pi 到 pi之间，归一化需要除以-pi到pi的概率密度和，但是这里为了方便便不做归一化
...
vec3 ReflectModelForHair(vec3 wo/*世界空间中的出射向量*/,vec3 n/*世界空间中的法向量*/){
	//to do 该模型较为繁琐，先就此告一段落，待后续再继续深入
	
	//头发模型的6个参数
	float h=0;//出射向量wo在圆柱的垂直直径上的投影点和半径的比例，范围为-1，1,其实就等于sinGamma_wo
	float eta = 1.5 / 1.0;//头发的折射率/空气的折射率
	float sigma_a = 0.1;//头发的吸收系数
	float beta_m = 0.1;//和圆柱界面夹角相关的粗糙度，范围0，1
	float beta_n = 0.1;//和圆柱中心线夹角相关的粗糙度，范围0，1
	float alpha = 2 / 180;//头发最外层鞘和圆柱边缘母线的夹角，一般为2度,这里用弧度表示
	...
	//根据alongD和n构建本地坐标系，其中alongD作为z，n作为x
	vec3 x = normalize(n);
	...
	//计算一些通用参数
	//计算wo和法平面夹角的相关值，以及wo在法平面投影和法向量夹角的相关值
	...
	for(uint sampleIndex = 0;sampleIndex < numSample;sampleIndex++)
	{
		...
		if(p != 3)
		{
			phi_i = Phi(p,phi_o,phi_t)/*入射光的理论phi值*/ + FittingInverse(samplePoint.x) * 2/*满足分布的采样值，这里应该通过Np的inverse CDF来获得，但比较麻烦这里直接使用一个拟合的*/;//
		}else 
		{
		...
		if(p != 3)
		{
			theta_i = -theta_o/*入射光的理论theta值*/ + FittingInverse(samplePoint.y)/*满足分布的采样值，这里应该通过Mp的inverse CDF来获得，但比较麻烦这里直接使用一个拟合的*/;//
		}else 
		{
		...
		float curAp = Ap[p];
		float curMp = Mp(sinTheta_i,cosTheta_i,sinTheta_o,cosTheta_o,0.2/*这里应该和beta_m相关，但这里先直接定义为0.2*/);
		...
		if(false)
		{
			//当前的bsdf项，这里要除以cosTheta_i,由于可能为0，所以这里只是给出该模型的bsdf的项由哪些构成，不参与最终计算
